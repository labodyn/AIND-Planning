...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        """
        old_s_level = self.s_levels[level]
        action_set = set()
        for action in self.all_actions:
            a_node = PgNode_a(action)
            if a_node.prenodes.issubset(old_s_level):
                for s_node in old_s_level:
                    s_node.children.add(a_node)
                    a_node.parents.add(s_node)
                action_set.add(a_node)
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level - 1]:
            for effect_node in a_node.effnodes:
                ...
                state_set.add(effect_node)
        self.s_levels.append(state_set)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        if set(node_a1.action.effect_rem).intersection(node_a2.action.effect_add):
            return True
        if set(node_a2.action.effect_rem).intersection(node_a1.action.effect_add):
            return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        if set(node_a1.action.effect_rem).intersection(node_a2.action.precond_pos):
            return True
        if set(node_a1.action.effect_add).intersection(node_a2.action.precond_neg):
            return True
        if set(node_a2.action.effect_rem).intersection(node_a1.action.precond_pos):
            return True
        if set(node_a2.action.effect_add).intersection(node_a1.action.precond_neg):
            return True
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        # PgNode_s is missing methods to implement this in oop way.
